...
const char *HTML_BODY = "<body><div id='menu'><button id='menuOption1'>Some button</button><button id='menuOption2'>Some image</button><button id='menuOption3'>Some text</button></div><div id='content'></div><img id='img' style='object-fit: cover; width:800px;'>";
const char *HTML_BOTTOM = "<script>$(document).ready(function() { const buttonAJAX = document.getElementById('menuOption1');buttonAJAX.addEventListener('click', loadAJAX);function loadAJAX() { var xhr = new XMLHttpRequest();xhr.open('GET', '/AJAX_request', true); xhr.onload = function() {if (this.status == 200) console.log(this.responseText); else console.log(this.status); };};"
                          ...
                          "function getCameraImage() { jQuery.get('AJAX_request', null, data => { $('#img').attr('src', 'data:image/png;base64,'+data);"
                          "performance[1] = new Date(); getCameraImage(); performance[2] = new Date(); fps++; console.log('Request received after '+(performance[1] - performance[0])+' ms and processed after '+(performance[2] - performance[0]) + ' ms'); performance[0] = performance[2]; } ); }"
                          "var fps = 0; setInterval(_ => { console.log('FPS: '+fps); fps = 0; }, 1000); getCameraImage();"
                          "});</script></body></html>";
